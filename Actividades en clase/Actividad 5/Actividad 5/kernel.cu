...
__global__ void add_vectors(int* a, int* b, int* c, int size) {
    ...
    for (int x = 0; x < 4; x++) {
        c[row * MATRIZ_SIZE + col] += a[row * MATRIX_SIZE + x] + b[x * MATRIX_SIZE + col];
    }
}

int main()
{
    const int matrix_size = MATRIX_SIZE * 1;
    dim3 blockSize(3, 3);
    ...
    // Lanzar el kernel
    add_vectors << <gridSize, blockSize >> > (a_device, b_device, c_device, matrix_size);
